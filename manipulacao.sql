...
values ('Feijoada', 'Feijoada completa com arroz, farofa, couve e laranja.', 29.90, 'Prato Principal'),
...
-- Inserindo em info_produtos
insert into info_produtos (id_produto,ingredientes,fornecedor)
values (1, 'Feijão preto, carne seca, costelinha, linguiça, arroz, farinha, couve, laranja', 'Distribuidora Central'),
...
-- Inserindo em pedidos
insert into pedidos (id_cliente,id_funcionario,id_produto,quantidade,data_pedido,operação)
values (1, 4, 1, 2, '2024-07-01', 'Concluído'),
...
-- Desativando Safe (Obs: Eu reativei caso de falha acione novamente!!)
...
select nome,telefone from clientes where year (data_nascimento) < '1985';

-- Selecionando os ID dos produtos
select id_produto,ingredientes from info_produtos where ingredientes like '%Carne%';

-- Deixando em ordem os produtos
select nome,categoria from produtos order by nome,categoria asc;

-- Limitando alguns produtos
select nome,descricao,categoria,preco from produtos order by preco desc limit 5;

-- Deixando apenas o 'Prato principal'
select nome,descricao,categoria,preco from produtos where categoria = "Prato Principal" limit 2 offset 6;
